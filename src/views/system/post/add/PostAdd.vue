...
import type { AddPostInfoParams } from '@/api/system/post/type'
import { addPostApi } from '@/api/system/post'

const btnLoading = ref(false)
const formRef = ref<FormInstance>()
const form = ref<AddPostInfoParams>({
  postName: '',
  postCode: '',
  sortNum: 0,
  status: 0,
...
const handleConfirm = () => {
  ...
  formRef.value.validate(async valid => {
    if (valid) {
      ...
      console.log('submit!')
      const result = await addPostApi(form.value).catch(e => e)
      if (result.code === 200) {
