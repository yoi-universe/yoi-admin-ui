...
import type { UpdateRoleInfoParams } from '@/api/system/role/type'
import { getRoleInfoApi, updateRoleApi } from '@/api/system/role'
...
const form = ref<UpdateRoleInfoParams>({
  roleId: 0,
  roleName: '',
  roleKey: '',
  sortNum: 0,
...
const getData = async () => {
  loading.value = true
  const roleInfoRes = await getRoleInfoApi(
    idList.value[currentIndex.value],
  ).catch(e => e)
  if (roleInfoRes.code === 200) {
    form.value = roleInfoRes.data
  }
  loading.value = false
}
...
const handleConfirm = () => {
  ...
  formRef.value.validate(async valid => {
    if (valid) {
      ...
      console.log('submit!')
      const result = await updateRoleApi(form.value).catch(e => e)
      if (result.code === 200) {
